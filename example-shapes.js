  ...
  { 'populate': function()        // vector, and texture-space coordinate.
      {
         ...
         this.texture_coords = [ vec2(1,1),   vec2(1,0),   vec2(0,1)   ];   // ...
...
Declare_Any_Class( "Square",    // A square, demonstrating shared vertices.  On any planar surface, the interior edges don't make any important seams.
  { 'populate': function()      // In these cases there's no reason not to re-use values of the common vertices between triangles.  This makes all the
      {                         // vertex arrays (position, normals, etc) smaller and more cache friendly.
         this.positions     .push( vec3(-.5,-.5,0), vec3(.5,-.5,0), vec3(-.5,.5,0), vec3(.5,.5,0) ); // Specify the 4 vertices -- the point cloud that our Square needs.
         this.normals       .push( vec3(-.3,-1,.2), vec3(1,-1,.2), vec3(-.6,.2,-.3), vec3(1,0,1) );     // ...
...
Declare_Any_Class( "Rectangle",    // A square, demonstrating shared vertices.  On any planar surface, the interior edges don't make any important seams.
  { 'populate': function(height, width)      // In these cases there's no reason not to re-use values of the common vertices between triangles.  This makes all the
      {                         // vertex arrays (position, normals, etc) smaller and more cache friendly.
         this.positions     .push( vec3(-width,-height,0), vec3(width,-height,0), vec3(-width,height,0), vec3(width,height,0) ); // Specify the 4 vertices -- the point cloud that our Square needs.
         this.normals       .push( vec3(0,.4,.6), vec3(1,0,0), vec3(0,.2,1), vec3(0,0,1) );     // ...
         this.texture_coords.push( vec2(0,0),   vec2(1,0),   vec2(0,1),   vec2(1,1)   );     // ...
         this.indices       .push( 0, 1, 2,     1, 3, 2 );                                   // Two triangles this time, indexing into four distinct vertices.
      }
  }, Shape )
...
// *********** Cube ***********
// Declare_Any_Class( "Cube",              // A demo of flat vs smooth shading.  Also our first 3D, non-planar shape.
//   { 'populate': function()
//       {
//         this.positions.push( vec3(0,0,0), vec3(1,0,0), vec3(1,0,1), vec3(0,0,1), vec3(0,1,1), vec3(0,1,0), vec3(1,1,0), vec3(1,1,1)  );

//         this.normals.push( vec3(0,0,0), vec3(1,0,0), vec3(1,0,1), vec3(0,0,1), vec3(0,1,1), vec3(0,1,0), vec3(1,1,0), vec3(1,1,1)  );
